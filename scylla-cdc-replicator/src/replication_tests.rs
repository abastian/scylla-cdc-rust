...
mod tests {
    ...
    async fn replicate(
        ...
        for mut log in result.rows.unwrap_or_default() {
            // handling udt specific
            for col_opt in log.columns.iter_mut() {
                let new_udt = if let Some(col) = col_opt {
                    match col {
                        UserDefinedType { keyspace:_, type_name, fields } => {
                            Some(UserDefinedType { keyspace: ks_dst.to_string(), type_name: type_name.clone(), fields: fields.clone() })
                        }
                        _ => None
                    }
                } else {
