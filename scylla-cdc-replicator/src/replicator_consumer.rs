...
impl PrecomputedQueries {
    ...
    async fn insert_value(&mut self, values: impl SerializeRow, timestamp: i64) -> anyhow::Result<()> {
        run_prepared_statement(
            &self.destination_table_params.session,
    ...
    async fn delete_row(&mut self, values: impl SerializeRow, timestamp: i64) -> anyhow::Result<()> {
        run_prepared_statement(
            &self.destination_table_params.session,
    ...
    async fn update_list<'a>(
        ...
        if let Some(added_elements) = data.get_value(column_name) {
            ...
            for (key, val) in added_elements {
                ...
                let values_for_update: &[Option<&'a CqlValue>] = values_for_update;
                self.precomputed_queries
